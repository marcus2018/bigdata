...
public abstract class StringVerifyUtils {
    ...
    /**
     * 匹配email地址(^([a-z0-9A-Z]+[-|\\.]?)+[a-z0-9A-Z]@([a-z0-9A-Z]+(-[a-z0-9A-Z]+)?\\.)+[a-zA-Z]{2,}$) 格式: XXX@XXX.XXX.XX 匹配 : foo@bar.com 或
     * foobar@foobar.com.au 不匹配: foo@bar 或 $$$@bar.com
     */
    ...
    /**
     * 不包括特殊字符的匹配 (字符串中不包括符号 数学次方号^ 单引号' 双引号" 分号; 逗号, 帽号: 数学减号- 右尖括号> 左尖括号< 反斜杠\ 即空格,制表符,回车符等 ) 格式为: x 或 一个一上的字符 匹配 : 不匹配:
     */
    public static final String non_special_char_regexp = "^[^'\"\\;,:-<>\\s].+$";

    /**
     * 判断是不是合法字符 c 要判断的字符 格式 : 匹配 : 不匹配:
     */
