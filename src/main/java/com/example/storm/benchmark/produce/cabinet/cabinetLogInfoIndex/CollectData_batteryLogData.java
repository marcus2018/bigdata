...
public class CollectData_batteryLogData extends BaseWindowedBolt {
   ...
   public  static final String MIN="min";
    ...
    public void prepare(Map stormConf, TopologyContext context,
        ...
        if(pool==null){
            synchronized (CollectData_batteryLogData.class){
                if(pool==null){
    ...
    public void execute(TupleWindow inputWindow) {
        ...
        Map<String,Long> mapBidRedisIndex=new HashMap<>();
        ...
        if(setBid.size()>0){
            ...
            for (int i=0;i<redisValues.size();i++){
                ...
                mapBidRedisIndex.put(redisBid,Long.parseLong(redisEventType));
                listBatteryLogInfo.add(new BatteryLogInfo(redisBid, Long.parseLong(redisEventType), redisTime,CollectData_batteryLogData.MAXINDEX));//redis最大的游标
       ...
       // List<BatteryLogOmitInfo> list=new ArrayList();
        ...
        for (int i=0;i<listBatteryLogInfo.size()-1;i++){
            ...
            if(bid1.equals(bid2)){  //同一个电池
              ...
              long redisIndex=mapBidRedisIndex.get(listBatteryLogInfo.get(i).getBid())==null?0:mapBidRedisIndex.get(listBatteryLogInfo.get(i).getBid());
            /*  if(listBatteryLogInfo.get(i).getOmitLogIndex()!=null){
              ...
              if(diff>1&&(listBatteryLogInfo.get(i+1).getEventType()>=redisIndex&&!CollectData_batteryLogData.MAXINDEX.equals(listBatteryLogInfo.get(i+1).getOmitLogIndex())
