...
public class Scooter_battery_config {

    private static final Logger logger = LoggerFactory.getLogger(Scooter_battery_config.class);
    public static void main( String[] args )
    {
        String mode = "Remote";  // 使用本地测试模式
        String conumser_group_name = "produce_scooter_battery_numbers_and_track";   // 每个Topology 需要设定唯一的 consumer group 名字，不能为空，支持 [a-z][0-9] 和 '_'，'-'，长度在 [3-63] 字符，只能以小写字母和数字开头结尾
        String project = "ec-hb";    // 日志服务的Project
        String logstore = "ec-hb";   // 日志服务的Logstore
       // String endpoint = "https://cn-shenzhen.log.aliyuncs.com";   // 日志服务访问域名
        ...
        builder.setBolt("slidingwindowbolt", new Scooter_battery_bolt()
                .withWindow(new BaseWindowedBolt.Duration(5, TimeUnit.SECONDS)   ,           new BaseWindowedBolt.Duration(5, TimeUnit.SECONDS)),2)
                .setNumTasks(4)
                .shuffleGrouping("spout");
/*
        builder.setBolt("countwordbolt", new FullPortCount()
           //     .withWindow(new BaseWindowedBolt.Duration(10, TimeUnit.SECONDS)   ,           new BaseWindowedBolt.Duration(10, TimeUnit.SECONDS)),2)

               .withWindow(new BaseWindowedBolt.Count(2), new BaseWindowedBolt.Count(2)),1)
                //.fieldsGrouping("slidingwindowbolt",new Fields("key"));
        ...
        if (mode.equals("Local")) {
            ...
            try {
                try {
                    StormSubmitter.submitTopology("produce_scooter_battery_numbers_and_track", conf, builder.createTopology());
