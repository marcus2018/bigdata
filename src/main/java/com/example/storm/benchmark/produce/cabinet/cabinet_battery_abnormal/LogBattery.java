...
public class LogBattery {
    private static final Logger logger = LoggerFactory.getLogger(LogBattery.class);
    public static void main( String[] args )
    {
        String mode = "Remote";  // 使用本地测试模式
        String conumser_group_name = "produce_cabinet_battery_info";   // 每个Topology 需要设定唯一的 consumer group 名字，不能为空，支持 [a-z][0-9] 和 '_'，'-'，长度在 [3-63] 字符，只能以小写字母和数字开头结尾
        String project = "collectdata";    // 日志服务的Project
        String logstore = "station_battery_detail";   // 日志服务的Logstore
        ...
        builder.setBolt("cabinetBatteryInfo", new LogBatterySlidingWindowBolt()
       ...
       Config.registerSerialization(conf, LogGroupData.class, LogGroupDataSerializSerializer.class);
        if (mode.equals("Local")) {
          //  logger.info("Local mode...");
            ...
            try {
                try {
                    StormSubmitter.submitTopology("produce_cabinet_battery_info", conf, builder.createTopology());
