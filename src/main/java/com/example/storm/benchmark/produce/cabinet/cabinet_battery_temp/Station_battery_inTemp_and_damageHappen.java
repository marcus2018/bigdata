...
public class Station_battery_inTemp_and_damageHappen {
    private static final Logger logger = LoggerFactory.getLogger(Station_battery_inTemp_and_damageHappen.class);
    public static void main( String[] args )
    {
        String mode = "Remote";  // 使用本地测试模式
        String conumser_group_name = "produce_batteryInStationTemp_and_damageHappen";   // 每个Topology 需要设定唯一的 consumer group 名字，不能为空，支持 [a-z][0-9] 和 '_'，'-'，长度在 [3-63] 字符，只能以小写字母和数字开头结尾
        ...
        builder.setBolt("slidingwindowbolt", new Station_battery_inTemp_and_damageHappenBolt()
                .withWindow(new BaseWindowedBolt.Duration(5, TimeUnit.SECONDS)   ,           new BaseWindowedBolt.Duration(5, TimeUnit.SECONDS)),2)
                .shuffleGrouping("spout");

 /*      builder.setBolt("countwordbolt", new LogBatteryInStationTemp()
               .withWindow(new BaseWindowedBolt.Count(2), new BaseWindowedBolt.Count(2)),2)
       ...
       Config.registerSerialization(conf, LogGroupData.class, LogGroupDataSerializSerializer.class);
        if (mode.equals("Local")) {
          //  logger.info("Local mode...");
            ...
            try {
                try {
                    StormSubmitter.submitTopology("produce_batteryInStationTemp_and_damageHappen", conf, builder.createTopology());
